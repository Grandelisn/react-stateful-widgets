...
export default function Programmers() {
  ...
  const [featName, setFeatName] = useState(null);
  const getNameOfFeatured = () => {
    ...
    if(featName){
      const findName = name.find(findName => findName.id === featName);
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color:`${featName ? 'gold' : 'royalblue'}`, // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfAwesome.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setFeatName(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
